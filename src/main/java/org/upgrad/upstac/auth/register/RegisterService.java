...
public class RegisterService {
    ...
    public User addUser(RegisterRequest registerRequest) {
        ...
        if((null != userService.findByUserName(registerRequest.getUserName())))
            throw new AppException("User already exists " + registerRequest.getUserName());

        if((null != userService.findByEmail(registerRequest.getEmail())))
            throw new AppException("Email already registered " + registerRequest.getEmail());


        if((null != userService.findByPhoneNumber(registerRequest.getPhoneNumber())))
            throw new AppException("Phone number already associated with an user " + registerRequest.getPhoneNumber());
    ...
    public User addDoctor(RegisterRequest user) {
        ...
        if((null != userService.findByUserName(user.getUserName())))
            throw new AppException("User already exists " + user.getUserName());

        if((null != userService.findByEmail(user.getEmail())))
            throw new AppException("Email already registered" + user.getEmail());


        if((null != userService.findByPhoneNumber(user.getPhoneNumber())))
            throw new AppException("Phone number already associated with an user" + user.getPhoneNumber());
